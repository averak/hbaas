...
var (
	userAllColumns            = []string{"id", "email", "status", "is_deleted", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"id", "email", "status", "is_deleted", "created_at", "updated_at"}
...
// loaded structs of the objects. This is for a 1-1 relationship.
func (userL) LoadUserAuthentication(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`user_authentications`),
		qm.WhereIn(`user_authentications.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load UserAuthentication")
	}

	var resultSlice []*UserAuthentication
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice UserAuthentication")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for user_authentications")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for user_authentications")
	}
	...
	if singular {
		foreign := resultSlice[0]
		object.R.UserAuthentication = foreign
		if foreign.R == nil {
			foreign.R = &userAuthenticationR{}
		}
		foreign.R.User = object
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.ID == foreign.UserID {
				local.R.UserAuthentication = foreign
				if foreign.R == nil {
					foreign.R = &userAuthenticationR{}
				}
				foreign.R.User = local
...
func (o *User) SetUserAuthentication(ctx context.Context, exec boil.ContextExecutor, insert bool, related *UserAuthentication) error {
	...
	if o.R == nil {
		o.R = &userR{
			UserAuthentication: related,
		}
	} else {
		o.R.UserAuthentication = related
	}
...
func (o UserSlice) InsertAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "dao: unable to insert all from user slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dao: failed to get rows affected by insertall for users")
	}
...
func (o UserSlice) UpsertAll(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) (int64, error) {
	...
	nzDefaults := queries.NonZeroDefaultSet(userColumnsWithDefault, o[0])
	...
	if updateOnConflict && len(update) == 0 {
		return 0, errors.New("dao: unable to upsert users, could not build update column list")
	}
	...
	if err != nil {
		return 0, errors.Wrap(err, "dao: unable to upsert for users")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dao: failed to get rows affected by upsert for users")
	}
