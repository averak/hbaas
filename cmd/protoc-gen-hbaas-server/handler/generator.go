...
var (
	...
	methodInfoIdent      = protogen.GoImportPath("github.com/averak/hbaas/app/infrastructure/connect/advice").Ident("MethodInfo")
	reqIdent             = protogen.GoImportPath("github.com/averak/hbaas/app/infrastructure/connect/advice").Ident("Request")
)
...
func (g Generator[MethodOpt]) Generate(plugin *protogen.Plugin, file *protogen.File) error {
	...
	filename := file.GeneratedFilenamePrefix + ".hbaas.go"
...
func (g Generator[MethodOpt]) generateHandler(gf *protogen.GeneratedFile, service *protogen.Service) {
	gf.P(service.Comments.Leading, "type hbaas_", service.GoName, "Handler interface {")
	for _, method := range service.Methods {
...
func (g Generator[MethodOpt]) generateHandlerImpl(gf *protogen.GeneratedFile, service *protogen.Service) {
	methodInvokeIdent := protogen.GoImportPath("github.com/averak/hbaas/app/infrastructure/connect").Ident("Execute")
	...
	gf.P("handler hbaas_", service.GoName, "Handler")
...
func (g Generator[MethodOpt]) generateHandlerConstructor(gf *protogen.GeneratedFile, service *protogen.Service, file *protogen.File) {
	...
	newMethodInfoIdent := protogen.GoImportPath("github.com/averak/hbaas/app/infrastructure/connect/advice").Ident("NewMethodInfo")

	gf.P("func New", service.GoName, "Handler(handler hbaas_", service.GoName, "Handler, adv ", g.conf.AdviceIdent, ") ", handlerImplName(service), " {")
...
func handlerImplName(service *protogen.Service) string {
	return "hbaas_" + service.GoName + "HandlerImpl"
}
